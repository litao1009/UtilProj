...
public:
    ...
    virtual ::grpc::Status SayHello(::grpc::ServerContext* context, const ::helloworld::HelloRequest* request, ::helloworld::HelloReply* response) override
    {
        ...
        response->set_message(message);
        {
            ...
            std::cout << std::this_thread::get_id() << ">>>>Server:Recv[" << message << "]" << std::endl;
        }
...
void	RunSyncClient()
{
    ...
    {
        std::unique_lock<std::mutex> lock(g_mutex);
        std::cout << ">>SyncClint:PreSend" << std::endl;
    }
    auto status = stub->SayHello(&context, req, &reply);
    {
        std::unique_lock<std::mutex> lock(g_mutex);
        std::cout << ">>SyncClint:PostSend" << std::endl;
    }
}
            ...
            WaitThreadList_.emplace_back([this]()
            {
                ...
                while ( Run_ && CQ_->Next(&tag, &ok) )
                {
                    ...
                    if ( ok )
                    {
                        ...
                        std::unique_lock<std::mutex> lock(g_mutex);
                        std::cout << std::this_thread::get_id() << " Client:" << successTag << " " << ptask->Reply.message() << std::endl;
                    }
...
public:
    ...
    void    Init()
    {
        ...
        {
            std::unique_lock<std::mutex> lock(g_mutex);
            std::cout << std::this_thread::get_id() << ">>>>Server:Init Finish" << std::endl;
        }
    }
    ...
    void    Process(Task* const task)
    {
        ...
        case Task::ES_Accept:
        {
            ...
            {
                std::unique_lock<std::mutex> lock(g_mutex);
                std::cout << std::this_thread::get_id() << ">>>>Server:ES_Accept" << std::endl;
            }
        }
        break;
        case Task::ES_Request:
        {
            ...
            {
                std::unique_lock<std::mutex> lock(g_mutex);
                std::cout << std::this_thread::get_id() << ">>>>Server:ES_Request[" << message << "]" << std::endl;
            }
        ...
        case Task::ES_Reply:
        {
            {
                std::unique_lock<std::mutex> lock(g_mutex);
                std::cout << std::this_thread::get_id() << ">>>>Server:ES_Reply[" << task->Reply.message() << "]" << std::endl;
            }
    ...
    void    Stop()
    {
        ...
        {
            std::unique_lock<std::mutex> lock(g_mutex);
            std::cout << std::this_thread::get_id() << ">>>>Server:Stop" << std::endl;
        }
    }
};
...
int main()
{
    //std::thread serverTD(RunSyncServer);
    AsyncServer::GetInstance().Init();
    AsyncServer::GetInstance().Run();
